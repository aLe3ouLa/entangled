...
async function main() {
  ...
  const charactersSrc = characters
    ...
    prominence: [${c.prominence.join(', ')}], // real data — from TMDb per-season billing order
  },`,
    )
    .join('\n');
...
// GENERATED by scripts/generate-series.mjs — the character roster,
// per-season prominence (real TMDb per-season billing order), and season
// synopses (real TMDb season overviews) are actual data. Everything else
// is a placeholder: house, bio, aliveUntil (death timing), and every
// relationship need hand-authoring — no API tracks any of that.
