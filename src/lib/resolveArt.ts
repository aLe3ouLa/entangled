...
import {
  getAggregateCast,
  isConfigured as tmdbConfigured,
  profileUrl,
} from "./tmdb";
...
export async function resolvePhotos(
  ...
  if (series.characterArtSource === "anilist") {
    ...
    for (const e of entries)
      next[e.id] =
        findCastMatch(cast, e.tmdbCharacterName ?? e.name)?.imageUrl ?? null;
    return next;
  }
  ...
  const next: Record<string, string | null> = {};
  for (const e of entries)
    next[e.id] = profileUrl(
      findCastMatch(cast, e.tmdbCharacterName ?? e.name)?.profilePath,
    );
  return next;
}
